...
(function () {
  ...
  const log = (...args) => console.log('[reader-mode]', ...args);
  const warn = (...args) => console.warn('[reader-mode]', ...args);
  ...
  function watchAndApply(apply) {
    ...
    const obs = new MutationObserver(() => {
      try { apply(); } catch (e) { warn('apply error:', e); }
    });
  ...
  function applyCsdn() {
    ...
    const removeLoginWall = () => {
      const walls = document.querySelectorAll(
        '.hide-article-box, .readmore_box, #passportbox, .login-mark'
      );
      walls.forEach(el => { el.remove(); });

      // 解除"展开阅读全文"对正文的 height/overflow 限制
      const content = document.querySelector(
        '#article_content, .blog-content-box, .markdown_views, #content_views'
      );
      if (content) {
  ...
  function applyZhihu() {
    ...
    const closeLoginModal = () => {
      const modals = document.querySelectorAll(
        '.Modal-wrapper, .signFlowModal, .Modal--default, .LoginModal'
      );
    ...
    const expandCollapsed = () => {
      ...
      const collapsed = document.querySelectorAll(
        '.RichText.ztext.is-collapsed, .RichText--collapsed'
      );
      ...
      const expandBtns = document.querySelectorAll(
        'button.AnswerItem-expandButton, .ContentItem-more, [class*="Button"][class*="expand"]'
      );
      expandBtns.forEach(btn => { try { btn.click(); } catch (e) {} });
    };
