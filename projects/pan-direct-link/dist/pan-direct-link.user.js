    ...
    const Adapters = {
      baidu: {
        ...
        async fetchDirectLink(fileId) {
          ...
          const realFsId = ((_cached = cached) == null ? void 0 : _cached.fs_id) || fileId;
          const share = this._getShareParams();
          ...
          try {
            const resp = await fetch(`${baseUrl}/api/download?fidlist=[${realFsId}]&type=1&channel=chunlei&web=1&clienttype=0`, {
              credentials: 'include'
            });
            const text = await resp.text();
            try {
